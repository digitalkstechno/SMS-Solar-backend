  ...
  try {
    ...
    if (newStaff && oldStaff !== newStaff && (!req.user || newStaff !== String(req.user._id))) {
      ...
      await sendPushNotificationToUser(
        newStaff,
        "Lead Assigned",
        `You have been assigned to the lead: ${updatedLeads.fullName}`,
        { type: "lead", leadId: String(updatedLeads._id) }
      );
    }
    return res.status(200).json({
      status: "Success",
...
exports.mergeDuplicates = async (req, res) => {
  try {
    ...
    for (const lead of allLeads) {
      ...
      groups[normalized].push(lead);
    }

    let totalMergedGroups = 0;
    let totalDuplicatesRemoved = 0;

    for (const normalizedContact in groups) {
      ...
      for (const secondary of secondaryLeads) {
        // Merge followUps
        if (secondary.followUps && secondary.followUps.length > 0) {
          primaryLead.followUps = (primaryLead.followUps || []).concat(secondary.followUps);
          primaryUpdated = true;
        }
        // Merge attachments
        if (secondary.attachments && secondary.attachments.length > 0) {
          primaryLead.attachments = (primaryLead.attachments || []).concat(secondary.attachments);
          primaryUpdated = true;
        }
        // Merge activities
        if (secondary.activities && secondary.activities.length > 0) {
          primaryLead.activities = (primaryLead.activities || []).concat(secondary.activities);
          primaryUpdated = true;
        }
        // Merge payments
        if (secondary.payments && secondary.payments.length > 0) {
          primaryLead.payments = (primaryLead.payments || []).concat(secondary.payments);
          primaryUpdated = true;
        }
        // Merge quotations
        if (secondary.quotations && secondary.quotations.length > 0) {
          primaryLead.quotations = (primaryLead.quotations || []).concat(secondary.quotations);
          primaryUpdated = true;
        }
        // Merge assignedStock
        if (secondary.assignedStock && secondary.assignedStock.length > 0) {
          primaryLead.assignedStock = (primaryLead.assignedStock || []).concat(secondary.assignedStock);
          primaryUpdated = true;
        }
        ...
        fieldsToCopy.forEach(field => {
          if (!primaryLead[field] && secondary[field]) {
            primaryLead[field] = secondary[field];
            primaryUpdated = true;
          }
        });

        // Add activity record
        primaryLead.activities.push({
          message: `Merged details from duplicate lead: ${secondary.fullName || 'Unnamed'} (ID: ${secondary._id})`,
          by: req ? (req.user ? req.user._id : undefined) : undefined,
          date: new Date()
        });
        primaryUpdated = true;

        // Decrement status count for the duplicate lead status
        if (secondary.leadStatus) {
          await decrementCount({ statusId: secondary.leadStatus });
        }

        // Delete secondary lead
        await LEAD.findByIdAndDelete(secondary._id);
        totalDuplicatesRemoved++;
      }

      // Recalculate paymentAmount
      if (primaryLead.payments && primaryLead.payments.length > 0) {
        primaryLead.paymentAmount = primaryLead.payments.reduce((sum, p) => sum + (p.amount || 0), 0);
        primaryUpdated = true;
      }
    ...
    if (res) {
      return res.status(200).json({
        status: "Success",
        message: `Successfully merged duplicate leads.`,
        data: {
